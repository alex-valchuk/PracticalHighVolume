...
Write-Host ""
Write-Host "=== Creating specification: Phase 4 - Angular SPA (renumbered) ==="
Write-Host ""

# Clean up old spec if present
$oldSpec = "docs/specs/phase-4.5-angular-spa.md"
if (Test-Path -LiteralPath $oldSpec) {
    Remove-Item -LiteralPath $oldSpec -Force
    Write-Host "  - removed old docs/specs/phase-4.5-angular-spa.md"
}

$oldRedisSpec = "docs/specs/phase-4-redis-performance.md"
if (Test-Path -LiteralPath $oldRedisSpec) {
    $newRedisPath = "docs/specs/phase-5-redis-performance.md"
    Move-Item -LiteralPath $oldRedisSpec -Destination $newRedisPath -Force
    Write-Host "  ~ moved phase-4-redis-performance.md -> phase-5-redis-performance.md"
}

Write-File "docs/specs/phase-4-angular-spa.md" @'
...
- Advanced state management (NgRx) — deferred.
...
Write-Host ""
Write-Host "Review the spec:" -ForegroundColor Yellow
Write-Host "  notepad docs\specs\phase-4-angular-spa.md"
Write-Host ""
Write-Host "Phase map (final):" -ForegroundColor Yellow
Write-Host "  1 Domain + Clean Architecture"
Write-Host "  2 Integration with bookings (ACL)"
Write-Host "  3 Bookings module + Saga"
Write-Host "  4 Angular SPA         <-- this spec"
Write-Host "  5 Redis + Performance"
Write-Host "  6 Event-driven (RabbitMQ + Outbox)"
Write-Host "  7 Observability (OTel + Grafana + Prom)"
Write-Host "  8 Container + Kubernetes + Aspire"
Write-Host "  9 Polish + demo scenarios"
Write-Host ""
Write-Host "If approved:" -ForegroundColor Yellow
Write-Host "  git add ."
Write-Host '  git commit -m "docs(spec): SPEC-004 Angular SPA (phase 4)"'
Write-Host "  git push"
Write-Host ""